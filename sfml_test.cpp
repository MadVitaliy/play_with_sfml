...
sf::Image UniformQuantizedImageDithering(const sf::Image& i_image, size_t i_n = 3)
{
    union RGBA
    {
        sf::Uint32 rgba;
    ...
    const auto image_size = i_image.getSize();
    std::vector<double> pixel_data(image_size.x * image_size.y * sizeof(sf::Uint32));
    {
        const sf::Uint8* original_data = i_image.getPixelsPtr();
        for (size_t i = 0; i < pixel_data.size(); ++i)
            pixel_data[i] = original_data[i];

    }

    for (size_t y = 0; y < image_size.y - 1; ++y)
    {
        for (size_t x = 0; x < image_size.x - 1; ++x)
        {
            for (size_t c = 0; c < sizeof(sf::Uint32); c++)
            {
                double oldpixel = pixel_data[(y * image_size.x + x) * 4 + c];
                double& newpixel = pixel_data[(y * image_size.x + x) * 4 + c];
                newpixel = std::round(newpixel * (i_n / 255.)) * 255. / i_n;
                double error = oldpixel - newpixel;
                pixel_data[(y * image_size.x + x + 1) * 4 + c] += error * 7. / 16;
                pixel_data[((y + 1) * image_size.x + x - 1) * 4 + c] += error * 3. / 16;
                pixel_data[((y + 1) * image_size.x + x) * 4 + c] += error * 5. / 16;
                pixel_data[((y + 1) * image_size.x + x + 1) * 4 + c] += error * 1. / 16;

            }
        }
    }

    std::vector<sf::Uint8> new_pixel_data(image_size.x * image_size.y * sizeof(sf::Uint32));
    for (size_t i = 0; i < pixel_data.size(); ++i)
        new_pixel_data[i] = pixel_data[i];

    sf::Image res;
    res.create(image_size.x, image_size.y, reinterpret_cast<sf::Uint8*>(new_pixel_data.data()));
    return res;
}
...
int main(int argc, char** argv)
{
    ...
    timestamps.emplace_back(std::chrono::system_clock::now());
    const auto reduced_sorted_image = UniformQuantizedImageDithering(image, 6);
    timestamps.emplace_back(std::chrono::system_clock::now());
    /*
